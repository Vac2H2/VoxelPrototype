...
StructuredBuffer<int> _chunkIndexBuffer;
...
void GetPositionNormalUV_float(
    ...
    out float chunkIndex)
{
    int blockID = (uint)instanceID / 1024;

    int instanceCount = _instanceCountBuffer[blockID];
    ...
    chunkIndex = _chunkIndexBuffer[blockID] + 0.5;
    int3 worldPosition = _chunkPositionBuffer[chunkIndex];
...
void GetVoxelType_float(float chunkIndex, float3 localPosition, out float type)
{
    int3 pos = floor(localPosition);
    type = (float)_typeBuffer[floor(chunkIndex) * 32 * 32 * 32 + pos.x + 32 * pos.y + 32 * 32 * pos.z];
}
